...
export function ShelfViewPage() {
  ...
  const { useGetShelfById } = useShelf();
  const { data: shelf, isLoading } = useGetShelfById(id ?? '');
        ...
        <div className="flex items-start gap-3 md:gap-4">
          ...
          <div className="space-y-0.5 md:space-y-1">
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold tracking-tight flex items-center gap-2">
              ...
              Visualizar Prateleira
            </h1>
            <p className="text-xs md:text-sm text-muted-foreground">
              Detalhes completos da prateleira
            </p>
          </div>
        </div>
      </div>
          ...
          <Card className="border-0 shadow-none">
            <CardContent className="space-y-5 pt-6">
              ...
              <div className="space-y-2">
                ...
                <Input id="code" value={shelf.code} disabled />
              </div>

              {/* Nome */}
              <div className="space-y-2">
                <Label htmlFor="name">Nome</Label>
                <Input id="name" value={shelf.name} disabled />
              </div>
