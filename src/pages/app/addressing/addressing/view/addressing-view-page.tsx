import { useQuery } from '@tanstack/react-query';
import { ArrowLeft, Warehouse } from 'lucide-react';
import { useNavigate, useParams } from 'react-router-dom';

import { findAddressingById } from '@/api/stock/find-addressing-by-id';
import { findLocationById } from '@/api/stock/find-location-by-id';
import { findPositionById } from '@/api/stock/find-position-by-id';
import { findRowById } from '@/api/stock/find-row-by-id';
import { findShelfById } from '@/api/stock/find-shelf-by-id';
import { findSubLocationById } from '@/api/stock/find-sub-location-by-id';
import { fetchMaterialById } from '@/api/stock/fetch-materials';
import { Button } from '@/components/ui/button';
import { Card, CardContent } from '@/components/ui/card';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Skeleton } from '@/components/ui/skeleton';
import { Switch } from '@/components/ui/switch';

export function AddressingViewPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  const { data: addressing, isLoading } = useQuery({
    queryKey: ['addressing', id],
    queryFn: () => findAddressingById(id!),
    enabled: !!id,
  });

  const { data: location } = useQuery({
    queryKey: ['location', addressing?.locationId],
    queryFn: () => findLocationById(addressing!.locationId),
    enabled: !!addressing?.locationId,
  });

  const { data: subLocation } = useQuery({
    queryKey: ['subLocation', addressing?.subLocationId],
    queryFn: () => findSubLocationById(addressing!.subLocationId),
    enabled: !!addressing?.subLocationId,
  });

  const { data: row } = useQuery({
    queryKey: ['row', addressing?.rowId],
    queryFn: () => findRowById(addressing!.rowId),
    enabled: !!addressing?.rowId,
  });

  const { data: shelf } = useQuery({
    queryKey: ['shelf', addressing?.shelfId],
    queryFn: () => findShelfById(addressing!.shelfId),
    enabled: !!addressing?.shelfId,
  });

  const { data: position } = useQuery({
    queryKey: ['position', addressing?.positionId],
    queryFn: () => findPositionById(addressing!.positionId),
    enabled: !!addressing?.positionId,
  });

  const { data: material } = useQuery({
    queryKey: ['material', addressing?.materialId],
    queryFn: () => fetchMaterialById(addressing!.materialId!),
    enabled: !!addressing?.materialId,
  });

  const detailsLoading =
    isLoading ||
    (!!addressing && !location) ||
    (!!addressing && !subLocation) ||
    (!!addressing && !row) ||
    (!!addressing && !shelf) ||
    (!!addressing && !position);

  return (
    <div className="flex-1 space-y-4 md:space-y-6">
      {/* Header */}
      <div className="rounded-lg md:rounded-2xl border border-border/40 bg-card/50 backdrop-blur supports-backdrop-filter:bg-card/50 shadow-sm p-4 md:p-6 lg:p-8">
        <div className="flex items-start gap-3 md:gap-4">
          <Button
            variant="outline"
            size="icon"
            className="h-9 w-9 md:h-10 md:w-10 rounded-lg md:rounded-xl shadow-sm hover:shadow transition-all duration-200"
            onClick={() => navigate(-1)}
          >
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div className="space-y-0.5 md:space-y-1">
            <h1 className="text-xl md:text-2xl lg:text-3xl font-bold tracking-tight flex items-center gap-2">
              <Warehouse className="h-6 w-6" />
              Visualizar Endereçamento
            </h1>
            <p className="text-xs md:text-sm text-muted-foreground">
              Detalhes completos do endereçamento
            </p>
          </div>
        </div>
      </div>

      {detailsLoading ? (
        <div className="rounded-lg md:rounded-2xl border border-border/40 bg-card/50 backdrop-blur supports-backdrop-filter:bg-card/50 shadow-sm p-4 md:p-6 lg:p-8 space-y-5">
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-11 w-full" />
          <Skeleton className="h-16 w-full" />
        </div>
      ) : addressing && location && subLocation && row && shelf && position ? (
        <div className="rounded-lg md:rounded-2xl border border-border/40 bg-card/50 backdrop-blur supports-backdrop-filter:bg-card/50 shadow-sm p-4 md:p-6 lg:p-8">
          <Card className="border-0 shadow-none">
            <CardContent className="space-y-5 pt-6">
              {/* Localização */}
              <div className="space-y-2">
                <Label htmlFor="location">Localização</Label>
                <Input
                  id="location"
                  value={`${location.code} — ${location.name}`}
                  disabled
                />
              </div>

              {/* Sub-Localização */}
              <div className="space-y-2">
                <Label htmlFor="subLocation">Sub-Localização</Label>
                <Input
                  id="subLocation"
                  value={`${subLocation.code} — ${subLocation.name}`}
                  disabled
                />
              </div>

              {/* Fileira */}
              <div className="space-y-2">
                <Label htmlFor="row">Fileira</Label>
                <Input
                  id="row"
                  value={`${row.code} — ${row.name}`}
                  disabled
                />
              </div>

              {/* Prateleira */}
              <div className="space-y-2">
                <Label htmlFor="shelf">Prateleira</Label>
                <Input
                  id="shelf"
                  value={`${shelf.code} — ${shelf.name}`}
                  disabled
                />
              </div>

              {/* Posição */}
              <div className="space-y-2">
                <Label htmlFor="position">Posição</Label>
                <Input
                  id="position"
                  value={`${position.code} — ${position.name}`}
                  disabled
                />
              </div>

              {/* Material */}
              <div className="space-y-2">
                <Label htmlFor="material">Material</Label>
                <Input
                  id="material"
                  value={
                    material
                      ? `${material.code} — ${material.name}`
                      : 'Sem material'
                  }
                  disabled
                />
              </div>

              {/* Saldo */}
              <div className="space-y-2">
                <Label htmlFor="amount">Saldo</Label>
                <Input
                  id="amount"
                  type="number"
                  value={addressing.amount}
                  disabled
                />
              </div>

              {/* Status */}
              <div className="flex items-center justify-between p-4 bg-muted/30 rounded-lg">
                <div className="space-y-0.5">
                  <Label>Status do Endereçamento</Label>
                  <p className="text-sm text-muted-foreground">
                    {addressing.active
                      ? 'Endereçamento está ativo e pode ser utilizado'
                      : 'Endereçamento está inativo e não pode ser utilizado'}
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <span
                    className={`text-sm font-medium ${addressing.active ? 'text-green-600' : 'text-destructive'}`}
                  >
                    {addressing.active ? 'Ativo' : 'Inativo'}
                  </span>
                  <Switch checked={addressing.active} disabled />
                </div>
              </div>
            </CardContent>
          </Card>
        </div>
      ) : null}
    </div>
  );
}
