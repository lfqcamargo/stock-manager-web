...
export function PositionViewPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const { useGetPositionById } = usePosition();
  const { data: position, isLoading } = useGetPositionById(id ?? '');
          ...
          <Card className="border-0 shadow-none">
            <CardContent className="space-y-5 pt-6">
              ...
              <div className="space-y-2">
                ...
                <Input id="code" value={position.code} disabled />
              </div>

              {/* Nome */}
              <div className="space-y-2">
                <Label htmlFor="name">Nome</Label>
                <Input id="name" value={position.name} disabled />
              </div>
